...
class DetailedVisualizer(BaseVisualizer):
    def create_graph9(self, df, lifestyle, per_capita=False):
        ...
        fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(15, 15), 
                                           sharex=True, height_ratios=[3, 1, 1])
        
        # Plot 1: Food sacrifice
        sacrificing = (df_sorted['sacrifice'] < 0).sum()
        sacrificing_pct = (sacrificing / len(df_sorted)) * 100
        
        ax1.scatter(df_sorted['bucket_index'], df_sorted['sacrifice'],
                   color=self.colors[0], alpha=0.5, s=10, label='Food Sacrifice')
        ...
        plt.suptitle(f'Detailed Food Sacrifice Analysis - {lifestyle.capitalize()} {pop_type}')
    ...
    def create_graph10(self, df, lifestyle, per_capita=False):
        ...
        df_sorted['bucket'] = pd.qcut(range(len(df_sorted)), n_buckets, labels=False)
        ...
        bucket_stats['bucket_center'] = bucket_stats['bucket'] * bucket_size + bucket_size/2
        ...
        # Plot bucketed means
        line2 = ax2.plot(bucket_stats['bucket_center'], bucket_stats[f'c3{suffix}'],
                        color='green', linewidth=2, label='Mean Total Expenditure (c3)')
        line3 = ax3.plot(bucket_stats['bucket_center'], bucket_stats['persons_count'],
        ...
        plt.title(f'Detailed Food Sacrifice Analysis - {lifestyle.capitalize()} {pop_type}\n'
