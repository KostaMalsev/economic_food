...
class BucketingHelper:
    @staticmethod
    def create_fixed_width_buckets(df, value_column, max_value=None, bucket_size=250, min_samples=70):
        ...
        for bucket in range(1, int(bucket_counts.index.max() + 1)):  # Ensure integer range
            bucket_count = int(bucket_counts.get(bucket, 0))  # Ensure integer counts
        ...
        final_bucket_width = (max_val - min_val) / max(1, n_final_buckets)  # Prevent division by zero
            ...
            if isinstance(metric_info, dict):
                ...
                if metric_func and all(col in df.columns for col in required_cols):
                    try:
                        ...
                        stats[metric_name] = df.groupby(bucket_column).size() * 0  # Create zero-filled series
            else:
                try:
                    stats[metric_name] = df.groupby(bucket_column)[metric_info].agg(['mean', 'min', 'max', 'count'])
                except Exception as e:
                    print(f"Error calculating {metric_name}: {str(e)}")
